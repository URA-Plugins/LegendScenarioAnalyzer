...
internal sealed class LegendDisplayHistory(
    ...
    string panelTitle)
{
    ...
    static HistorySettings LoadSettings()
    {
        ...
        catch (JsonException ex)
        {
            throw new InvalidDataException(
                $"LegendScenarioAnalyzer 配置文件无效: {path}。{ex.Message}",
                ex);
        }
    ...
    static void ValidateHistoryLimit(int value, string source)
    {
        ...
        {
            throw new InvalidDataException(
                $"LegendScenarioAnalyzer historyLimit 必须在 0 到 {MaximumHistoryLimit} 之间: {source}，实际值 {value}。");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            Title = "LegendScenarioAnalyzer 配置",
            Width = 62,
            Height = 10,
        };
    ...
    {
        readonly LegendDisplayHistory owner;
        readonly IApplication application;
        View? content;
        bool active;

        internal HistoryView(LegendDisplayHistory owner, IApplication application)
        {
            this.owner = owner;
            this.application = application;
            Id = "legend-history-root";
            Width = Dim.Fill();
            Height = Dim.Fill();
            CanFocus = true;
        ...
        void ApplicationKeyDown(object? sender, TKey key)
        {
            ...
            if (contentNavigation is { } command && content is not null &&
                LegendTrainingDisplayRenderer.TryScroll(content, command))
            {
                key.Handled = true;
                return;
            }
    ...
    static string SettingsFilePath
        => Path.Combine("PluginData", "LegendScenarioAnalyzer", "settings.json");
